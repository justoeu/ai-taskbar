...
- **The Anthropic provider must default to read-only credentials**
  (`AnthropicConfig.manageOauthRefresh = false`, `AnthropicProvider`'s
  `manageOAuthRefresh` flag false in the shipped app via
  `AppEnvironment.makeProviders`). The app shares the `Claude Code-credentials`
  Keychain item with the Claude Code CLI, and Anthropic **rotates the refresh
  token on every exchange** — so refreshing it here invalidates the token other
  running CLI sessions hold (→ forced re-login) and trips the Keychain ACL
  prompt on ad-hoc builds. Read-only mode reads whatever token the CLI keeps
  current and lets the CLI own renewal; a briefly-expired token just serves the
  last cached snapshot. Only `manage_oauth_refresh = true` (opt-in, standalone
  use without the CLI) is allowed to call `AnthropicOAuth.refresh` +
  `writeBack`. Do not flip the default back to `true`.

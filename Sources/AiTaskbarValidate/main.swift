...
section("PricingTable lookup") {
    ...
    let gpt56 = PricingTable.lookup("gpt-5.6", table: PricingTable.openai)
    expect(gpt56?.inputPer1M == 5, "GPT-5.6 explicit entry (not prefix-dropped to gpt-5)")
    let gpt56Pro = PricingTable.lookup("gpt-5.6-pro", table: PricingTable.openai)
    expect(gpt56Pro?.inputPer1M == 30, "GPT-5.6 Pro longest-prefix wins over gpt-5.6")
    let gpt56Mini = PricingTable.lookup("gpt-5.6-mini", table: PricingTable.openai)
    expect(gpt56Mini?.inputPer1M == 0.75, "GPT-5.6 mini longest-prefix wins over gpt-5.6")

...
- When macOS blocks the read (`errSecInteractionNotAllowed`), the Anthropic tile shows a one-click **Authorize** banner: the app extends the item's partition list + trusted-app ACL itself (`KeychainAccessAuthorizer`) and macOS asks your login password **once** in a native dialog — no terminal, permanent. ("Always Allow" alone never sticks because it edits only the ACL, not the partition list.)
